...
module.exports = {
    ...
    copyFile: async function (options) {
        const provider = this.parseRequired(options.provider, 'string', 's3.copyFile: provider is required.');
        const srcBucket = this.parseRequired(options.srcBucket, 'string', 's3.copyFile: srcBucket is required.');
        const srcKey = this.parseRequired(options.srcKey, 'string', 's3.copyFile: srcKey is required.');
        const Bucket = this.parseRequired(options.dstBucket, 'string', 's3.copyFile: dstBucket is required.');
        const Key = this.parseRequired(options.dstKey, 'string', 's3.copyFile: dstKey is required.');
        const s3 = this.getS3Provider(provider);

        if (!s3) throw new Error(`S3 provider "${provider}" doesn't exist.`);

        const { CopyObjectCommand } = require('@aws-sdk/client-s3');
        const command = new CopyObjectCommand({ Bucket, Key, CopySource: `${srcBucket}/${srcKey}` });
